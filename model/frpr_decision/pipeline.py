...
class DecisionModel:
    ...
    M_PER_FOOT = 0.3048
    J_PER_KWH = 3.6e6
    STOP_INDEX = {"fort-collins": 0, "denver": 5, "colorado-springs": 9, "pueblo": 11}
    ...
    def facility_sizing(self, technology: Technology, battery_kwh_per_car: float = 0.0) -> tuple[tuple[FacilityCapacity, ...], float]:
        a = self.assumptions
        enabled_attr = "bemu_enabled" if technology.key == "bemu" else "hydrogen_enabled"
        ...
        for nodes in self.circuits(a.service_pattern):
            ...
            facility_indexes = [index for index, key in enumerate(nodes) if getattr(next(stop for stop in self.stops if stop.key == key), enabled_attr)]
        ...
        for stop in (item for item in self.stops if item.key in active_stop_keys and getattr(item, enabled_attr)):
            ...
            if technology.key == "bemu":
                kw = max_kwh / a.charging_efficiency / max(stop.dwell_minutes / 60, 1 / 60) * concurrency
